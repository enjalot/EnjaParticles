...
extern "C"
// GE: change name to avoid conflicts
void initDeviceParameters()
{
    ...
    if (cudaSuccess == cudaGetDevice(&deviceID))
    {
        ...
        for (int i = 0; i < SORT_KERNEL_COUNT; ++i)
        {
            ...
            numCTAs[i] = (unsigned int) (numSMs * MIN(ctaLimitRegs, MIN(ctaLimitSMem, MIN(ctaLimitThreads, 8))));
        }
    }
}
...
// ================================================================================================
__global__ void flipFloatsa(uint *values, uint numValues)
{
...
// ================================================================================================
// GE: change name to avoid conflicts
__global__ void unflipFloatsa(uint *values, uint numValues)
{
    uint index = __umul24(blockDim.x*4, blockIdx.x) + threadIdx.x; 
                   ...
                   uint numElements)
{
	...
	// on GT200, resulting in better scheduling and lower run times
        if (startbit > 0)
			// GE: change name to avoid conflicts
            emptyKernela<<<numCTAs[SORT_KERNEL_EMPTY], RadixSort::CTA_SIZE>>>();
    }
                          ...
                          uint numElements)
{
    ...
    if (flip)
			// GE: change name to avoid conflicts
            unflipFloatsa<<<1, RadixSort::CTA_SIZE>>>(keys, numElements);
