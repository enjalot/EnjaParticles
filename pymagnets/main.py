...
class window(object):
    ...
    def make_sph_system(self):
        ...
        ipos, icolor = initialize.addRect(512, Vec([.5, .5, 0.,0.]), Vec([1., 1., 0.,0.]), self.system, color)
    ...
    def on_click(self, button, state, x, y):
        if state == GLUT_DOWN:
            ...
            color = [rr, 0., rb, .5]
            self.clsystem.set_color(color)
            ipos, icolor = initialize.addRect(512, Vec([cx - .1, cy -.1, 0.1,0.]), Vec([cx + .1, cy + .1, 0.2,0.]), self.system, color)
            print "pushing clsystem particles"
            self.clsystem.push_particles(ipos, None, icolor)
            import time
