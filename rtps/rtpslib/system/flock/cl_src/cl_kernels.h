...
//----------------------------------------------------------------------
float Wpoly6(float4 r, float h, __constant struct FLOCKParams* params)
{
...
//----------------------------------------------------------------------
float Wpoly6_dr(float4 r, float h, __constant struct FLOCKParams* params)
{
...
//----------------------------------------------------------------------
float Wpoly6_lapl(float4 r, float h, __constant struct FLOCKParams* params)
{
// Laplacian
    float r2 = r.x*r.x + r.y*r.y + r.z*r.z;  // dist_squared(r);
	float h2 = h*h;
...
//----------------------------------------------------------------------
float Wspiky(float rlen, float h, __constant struct FLOCKParams* params)
{
...
//----------------------------------------------------------------------
float Wvisc_dr(float rlen, float h, __constant struct FLOCKParams* params)
// Derivative with respect to |r| divided by |r|
// 
{
	float alpha = 15./(2.*params->PI * h*h*h);
...
//----------------------------------------------------------------------
float Wvisc_lapl(float rlen, float h, __constant struct FLOCKParams* params)
{
	float h3 = h*h*h;
