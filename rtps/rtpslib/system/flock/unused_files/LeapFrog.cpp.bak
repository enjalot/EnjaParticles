...
//    k_leapfrog.setArg(iargs++, cl_color.getDevicePtr());
    k_leapfrog.setArg(iargs++, cl_FLOCKParams.getDevicePtr());
    ...
    k_leapfrog.setArg(4, cl_xflock.getDevicePtr());
    k_leapfrog.setArg(5, cl_color.getDevicePtr());
    k_leapfrog.setArg(6, ps->settings.dt); //time step
    k_leapfrog.setArg(7, cl_FLOCKParams.getDevicePtr());
    */
} 

void FLOCK::cpuLeapFrog()
{
    ...
    for(int i = 0; i < num; i++)
    {
        ...
        float xflockfactor = .1f;
        vnext.x += xflockfactor * xflocks[i].x;
        vnext.y += xflockfactor * xflocks[i].y;
        vnext.z += xflockfactor * xflocks[i].z;
