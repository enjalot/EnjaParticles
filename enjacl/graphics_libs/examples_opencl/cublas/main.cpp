...
extern void swanBindToTexture1D(const  char *texname, int width, void
*ptr, size_t typesize, int flags );
...
void tst_inv_diag_3d(int ntests)
{
	...
	****/
	...
	if (error == 0) {
		printf("inverseDiag3DKernel test: passed\n");
	} else {
	...
	if (error == 0) {
		printf("inverseDiag3DMulKernel test: passed\n");
	} else {
...
void tst_mat_mat_mul_el(int ntests)
{
	...
	for (int j=0; j < ny; j++) {
	for (int i=0; i < nx; i++) {
		if (fabs(res(i,j,k)-a(i,j,k)*b(i,j,k)) > 1.e-5) {
	...
	if (error == 0) {
		...
		printf("mat_mat_mul_el test not successful\n");
	}
#endif
}
//----------------------------------------------------------------------
void tstSvecvec(int ntests)
{
	...
	for (int j=0; j < 128; j++) {
	for (int i=0; i < 128; i++) {
		//if (fabs(res(i,j,k)-a(i,j,k)*b(i,j,k)) < 1.e-3) continue;
...
void tstScopy(int ntests)
{
	...
	if (err == 0) {
		...
		printf("scopy test unsuccessful\n");
	}
#endif
}
//----------------------------------------------------------------------
void tstSaxpy(int ntests)
{
	...
	for (int j=0; j < ny; j++) {
	for (int i=0; i < nx; i++) {
		if (fabs(alpha*a(i,j,k)+orig_b(i,j,k)-b(i,j,k)) > 1.e-5) {
			err = -1;
			printf("i,j,k,a,b= %d,%d,%d,   %g, %g\n", i,j,k, a(i,j,k), b(i,j,k));
			continue;
		}
	...
	if (err == 0) {
		...
		printf("saxpy test unsuccessful\n");
	}
#endif
}
//----------------------------------------------------------------------
void tstSdot(int ntests)
{
	...
	if (fabs(dot-a.getSize()) > 1.e-5) {
		printf("tstSdot failed\n");
	} else {
		printf("tstSdot succeeded\n");
	}
	printf("===========================\n");
#endif
}
...
void tstSscalParams(int ntests)
{
	...
	if (error == 0) {
		printf("sscalParams test succeeded\n");
	} else {
