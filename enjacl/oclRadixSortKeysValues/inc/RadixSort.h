...
private:
	...
	cl_mem d_tempValues;                 // Memory objects for original values and work space
										 // Added by GErlebacher 9/11/2010
    ...
    cl_kernel ckRadixSortBlocksKeysValues;// OpenCL kernels
	cl_kernel ckFindRadixOffsets;
	cl_kernel ckScanNaive;
	cl_kernel ckReorderDataKeysOnly;
	cl_kernel ckReorderDataKeysValues;
	...
	void radixSortStepKeysValues(cl_mem d_keys, cl_mem d_values, unsigned int nbits, unsigned int startbit, unsigned int numElements);
	void radixSortBlocksKeysOnlyOCL(cl_mem d_keys, unsigned int nbits, unsigned int startbit, unsigned int numElements);
	void radixSortBlocksKeysValuesOCL(cl_mem d_keys, cl_mem d_values, unsigned int nbits, unsigned int startbit, unsigned int numElements);
	...
	void reorderDataKeysValuesOCL(cl_mem d_keys, cl_mem d_values, unsigned int startbit, unsigned int numElements);
};
#endif
