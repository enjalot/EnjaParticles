...
__kernel void matrix_vec_polar_3d(__global float* x, __global float* y  ARGS_GLOB, __local float* sh)
#if 0
								...
								int zblock, __local float* sh ARGS_DUMMY)
{
	...
	VOLATILE int xi = (get_group_id(0) * blkDim.x) + get_local_id(0);
	VOLATILE int yi = (get_group_id(1) * blkDim.y) + get_local_id(1);
	VOLATILE int zi = (zblock     * blkDim.z) + get_local_id(2);
...
// size: is (block.x+2)*(block.y+2)*(block.z+2)
	...
	if (get_local_id(2) < 2) {
		...
		// 1st arg: 2, 3
		read_plane_polar_3d(get_local_id(2)+blkDim.z, x, sh, zblock ARGS_OCL);
	}
...
/*    for access into solution array (sol)  */
	...
	if (get_local_id(1) < 2) {
		...
		read_row_polar_3d(get_local_id(1)+blkDim.y, sol_glob, sha_loc ARGS_OCL);
	}
	_barrier(CLK_LOCAL_MEM_FENCE);
}
//----------------------------------------------------------------------
...
// arr: solution array (nx+2) x (ny+2) x (nz+2)
	...
	volatile int glob_row = row-1 + blkDim.y * get_group_id(1);  // blocks form partition of arr
...
//   shared memory cells correspond to cell centers
	...
	if (get_local_id(0) == 15) {
		if (get_group_id(0) != 0) {
			...
			sha_loc[0] = sol_row[get_group_id(0)*blkDim.x - 1]; // index must be > 0
		} else {
	...
	if (get_local_id(0) == 0) {  // any other threadIdx gives incoherent loads
		...
		if (get_group_id(0) != (get_num_groups(0)-1) ) {
			sha_loc[1+blkDim.x] = sol_row[get_group_id(0)*blkDim.x + blkDim.x]; // index must be < nx
		} else {
