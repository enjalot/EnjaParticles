...
//----------------------------------------------------------------------
extern "C" void copyToDeviceFromHost_1d(cl_mem, const void* src, size_t count);
extern "C" void copyToHostFromDevice(void* dst, const cl_mem src, size_t count);
extern "C" cl_mem openclMalloc_1d(int nbBytes);
// 2nd argument not required
extern "C" void clear_1d(cl_mem data, size_t count); // in bytes 
//----------------------------------------------------------------------

template <class T>
ArrayOpenCL1D<T>::ArrayOpenCL1D(T* array, int n1_, int n2_, int n3_, int n1m_, int n2m_, int n3m_) : ArrayOpenCLT<T>(array, n1_, n2_, n3_, n1m_, n2m_, n3m_)
{
	//printf("** array1D constructor: array, n1,n2,n3\n");
	//printf("new --- inside arrayOpenCL1D consructor array 2, nbChannels= %d\n", this->nbChannels);
	//this->isLocked = "";
...
//----------------------------------------------------------------------
template <class T>
ArrayOpenCL1D<T>::ArrayOpenCL1D(T* array, Vec3i& n_) : ArrayOpenCLT<T>(array, n_)
{
	//printf("inside arrayOpenCL1D consructor array 1\n");
	//this->isLocked = "";
...
//----------------------------------------------------------------------
template <class T>
ArrayOpenCL1D<T>::ArrayOpenCL1D(const Vec3i& n_) : ArrayOpenCLT<T>(n_)
{
	//printf("inside arrayOpenCL1D consructor 1\n");
	//this->isLocked = "";
	//createDeviceArray(this->nbChannels); // pure virtual called during the construction of the object? Illegal?
	createDeviceArray();
}
//----------------------------------------------------------------------
template <class T>
ArrayOpenCL1D<T>::ArrayOpenCL1D(int n1_, const char* locked) :
    ArrayOpenCLT<T>(n1_, 1, 1, 0, 0, 0)
{
	//printf("inside arrayOpenCL1D consructor 2, locked\n");
...
//----------------------------------------------------------------------
template <class T>
ArrayOpenCL1D<T>::ArrayOpenCL1D(int n1_, int n2_, int n3_, int n1m_, int n2m_, int n3m_) :
    ArrayOpenCLT<T>(n1_, n2_, n3_, n1m_, n2m_, n3m_)
{
	//this->isLocked = "";
	//printf("new ** -- inside arrayOpenCL1D consructor 3\n");
	//printf("channels: %d\n", this->nbChannels);
	createDeviceArray();
	//createDeviceArray(this->nbChannels); // pure virtual called during the construction of the object? Illegal?
	//this->init();
...
//----------------------------------------------------------------------
template <class T>
ArrayOpenCL1D<T>::~ArrayOpenCL1D()
{
	//cudaFree(this->d_data); // this-> is necessary due to templates
	clReleaseMemObject(this->d_data);
	//printf("inside arrayOpenCL1D destructor\n");
}
//----------------------------------------------------------------------
template <class T>
void ArrayOpenCL1D<T>::copyToDevice()
{
...
#if 0
template <class T>
void ArrayOpenCL1D<T>::copyDtoDfrom(ArrayOpenCL1D<T>& fromArray)
{
...
// Dangerous since pitching is dependent on the specific card. 
	...
	//printf("ArrayOpenCL2D: copyDtoD\n");
}
#endif
//----------------------------------------------------------------------
template <class T>
void ArrayOpenCL1D<T>::copyToHost()
{
...
//----------------------------------------------------------------------
template <class T>
void ArrayOpenCL1D<T>::createDeviceArray() // 1D, 2D, cuda
{
	int nbBytes = this->getSize() * sizeof(T);
	this->d_data = (cl_mem) openclMalloc_1d(nbBytes);
	//cudaMalloc_1d((void**) &this->d_data, nbBytes);

	if (this->d_data == 0) {
		printf("*** createDeviceArray:: cannot allocate memory (ArrayOpenCL1D) (%d) bytes\n", nbBytes);
		exit(0);
	}
}
//----------------------------------------------------------------------
#if 1
template <class T>
void ArrayOpenCL1D<T>::createDeviceArray(int nbChannels) // 1D, 2D, cuda
{
	int nbBytes = this->getSize() * sizeof(T);
	//printf("sizeof(T)= %d\n", (int) sizeof(T));
	//printf("nbBytes= %d\n", nbBytes);
	...
	// *** 153: error: lvalue required as unary ‘&’ operand
	//cudaMalloc_1d((cl_mem*) &this->getDevicePtr(), nbBytes);
	this->d_data = openclMalloc_1d(nbBytes);

	if (this->d_data == 0) {
		printf("*** createDeviceArray(%d):: cannot allocate memory (ArrayOpenCL1D) (%d) bytes\n", nbChannels, nbBytes);
		exit(0);
	}
	//printf("inside ArrayOpenCL1D<T>::createDeviceArray, d_data= %ld\n", (long) this->d_data);
}
#endif
//----------------------------------------------------------------------
#if 1
template <class T>
void ArrayOpenCL1D<T>::clear()
{
	...
	// data_d is cl_mem and not float*
	clear_1d(this->getDevicePtr(), this->getSize() * sizeof(T)); // count bytes
	//clear_1d(data_d, this->getSize() * sizeof(T)); // count bytes
}
#endif
//----------------------------------------------------------------------
#if 0
template <class T>
void ArrayOpenCL1D<T>::print(const char* msg, int xo, int yo, int w, int h)
{
	if (msg) {
	   printf("\n---------- %s -----------------\n", msg);
	}
