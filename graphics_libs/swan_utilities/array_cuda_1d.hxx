...
{
	if (this->d_data) {	
		swanFree(this->d_data); // this-> is necessary due to templates
	}
...
void ArrayCuda1D<T>::createDeviceArray() // 1D, 2D, cuda
{
	//try { throw 20; } 
	//catch (exception& e) {
		//printf("except: createDeviceArray()\n");
	//}

	int nbBytes = this->getSize() * sizeof(T);
	//printf("createDeviceArray 2\n");
	cudaMalloc_1d((void**) &this->d_data, nbBytes);
	//printf("ge: after cudaMalloc_1d\n");
